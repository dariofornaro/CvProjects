...
let rec getOptions edges node mem=
    match edges with
    |Edge(Node(q1),a,Node(q2))::xs when q1=node && ((defined a mem) = Intertrue) -> (q2,(change_memory a mem))::getOptions xs node mem
    |[] -> []
    |_::xs -> getOptions xs node mem


let rec Model_checker edges visited toExplore =
    match toExplore with
    |(a,b)::xs when Set.contains (a,b) visited -> Model_checker edges visited xs 
    |(a,b)::xs -> 
                  if (getOptions edges a b).IsEmpty then 
                    if a="qend" then printfn "Finished Execution:" else printfn "Stuck state found: "
                    printfn "node: %s " (a)
                    printfn "mem:  %A" (b)
                    printfn "\n"
                    Model_checker edges (Set.add (a,b) visited) xs 
                  else Model_checker edges (Set.add (a,b) visited) (toExplore @ (getOptions edges a b)) 
    |[] -> printfn "Done"
